...
class Rank6(object):
    ...
    def __init__(self, pin_1: int, pin_2: int, pin_3: int, pin_4: int,
                 pin_5: int, pin_6: int) -> object:
        # pin for lights
        self.pins = [Pin(pin_1, mode = Pin.OUT), Pin(pin_2, mode = Pin.OUT),
                     Pin(pin_3, mode = Pin.OUT), Pin(pin_4, mode = Pin.OUT),
                     Pin(pin_5, mode = Pin.OUT), Pin(pin_6, mode = Pin.OUT)]
...
class Node(object):
    ...
    def preamble(self) -> None:
        ...
        # turn off lights
        self.light.handle_cmd(Rank6.LIGHT_COUNT+1)
